...
def main(rank, args):
    ...
    vae_cond_ema = copy.deepcopy(vae_cond_model).to(device)
    update_ema(vae_cond_ema, vae_cond_model, decay=0)
    set_requires_grad(vae_cond_ema, False)
    vae_cond_ema.eval()
    
    vae_pred_ema = copy.deepcopy(vae_pred_model).to(device)
    update_ema(vae_pred_ema, vae_pred_model, decay=0)
    set_requires_grad(vae_pred_ema, False)
    vae_pred_ema.eval()
    ...
    vae_pred_criterion = nn.parallel.DistributedDataParallel(vae_pred_criterion, device_ids=[device], find_unused_parameters=True)
    vae_cond_model = nn.parallel.DistributedDataParallel(vae_cond_model, device_ids=[device], find_unused_parameters=False)
    vae_cond_criterion = nn.parallel.DistributedDataParallel(vae_cond_criterion, device_ids=[device], find_unused_parameters=True)
    ...
    losses['vae_cond_kl'] = AverageMeter()
    losses['vae_pred_kl'] = AverageMeter()
    ...
    while last_it < args.max_iter:
        ...
        x_vae = (x / 127.5 - 1).transpose(1, 2).contiguous() 
        
        x_cond = x_vae[:, :, :args.cond_frames] 
        x_pred = x_vae[:, :, args.cond_frames : args.frames]

        is_sync_step = (it % accum_iter == accum_iter - 1)

        # -------------------------------------------------------------
        # BLOCK A: 提取 Latents (保证后续的 DiT 训练永远有输入)
        # -------------------------------------------------------------
        if not disc_opt:
            ...
            if not is_sync_step:
                ...
                sync_context_g.enter_context(vae_pred_criterion.no_sync())
                sync_context_g.enter_context(vae_cond_criterion.no_sync())
            ...
            with sync_context_g:
                with torch.autocast(device_type='cuda', enabled=args.amp):
                    ...
                    x_cond_hat, kl_loss_cond, z_cond = vae_cond_model(x_cond, return_extract=True)
                    ...
                    x_target_hat, kl_loss_pred, z_pred = vae_pred_model(x_pred, return_extract=True)
                    vae_pred_loss, _, pred_kl_loss, pred_l1_recon = vae_pred_criterion(
                        kl_loss_pred, x_pred, rearrange(x_target_hat, '(b t) c h w -> b c t h w', b=batch_size),
                        optimizer_idx=0, global_step=it // accum_iter
                    )
            ...
            losses['vae_cond_recon'].update(cond_l1_recon.item(), 1)
            ...
            sync_context_d = contextlib.ExitStack()
            if not is_sync_step:
                sync_context_d.enter_context(vae_pred_criterion.no_sync())
                sync_context_d.enter_context(vae_cond_criterion.no_sync())
            ...
            with sync_context_d:
                with torch.autocast(device_type='cuda', enabled=args.amp):
                    ...
                    d_loss_pred = vae_pred_criterion(
                        vq_loss_pred, x_pred, rearrange(x_pred_tilde, '(b t) c h w -> b c t h w', b=batch_size), 
                        optimizer_idx=1, global_step=it // accum_iter)
                    d_loss_cond = vae_cond_criterion(
                        vq_loss_cond, x_cond, rearrange(x_cond_tilde, '(b t) c h w -> b c t h w', b=batch_size), 
                        optimizer_idx=1, global_step=it // accum_iter)
        ...
        with sync_context_fm:
            with torch.autocast(device_type='cuda', enabled=args.amp):
                ...
                dit_repa_val = sit_outputs.get("align_dit_loss", torch.tensor(0.0, device=device)).mean()
        ...
        # -------------------------------------------------------------
        # BLOCK D: 优化器步进 (统一管理，严格绑定 is_sync_step)
        # -------------------------------------------------------------
        if is_sync_step:
            if args.amp:
                if not disc_opt:
                    ...
                    scaler_d.step(opt_vae_d)
                    scaler_d.update()
                    opt_vae_d.zero_grad(set_to_none=True)
            ...
            current_decay = min(0.9999, (it // accum_iter + 1) / (it // accum_iter + 10))
            ...
            if not disc_opt:
                ...
                update_ema(vae_cond_ema, unwrapped_vae_cond, decay=current_decay)
                
                unwrapped_vae_pred = vae_pred_model.module if hasattr(vae_pred_model, 'module') else vae_pred_model
                update_ema(vae_pred_ema, unwrapped_vae_pred, decay=current_decay)

            # --- Discriminator ---
            if it // accum_iter >= disc_start:
        ...
        if rank == 0 and it % 10 == 0:
            if not disc_opt:
                ...
                pbar.set_description(f"Disc Loss: {losses['disc_loss'].average:.3f} | Denoise: {losses['dit_denoise_loss'].average:.3f}")
        ...
        if it % args.log_freq == 0:
            if rank == 0 and logger is not None:
                ...
                logger.scalar_summary('recon/vae_cond_kl', losses['vae_cond_kl'].average, it)
                logger.scalar_summary('recon/vae_pred_kl', losses['vae_pred_kl'].average, it)
        ...
        if rank == 0 and (it + 1) % args.eval_freq == 0 and it > 0:
            ...
            with torch.no_grad():
                try:
                    ...
                    unwrapped_vae_cond_ema = vae_cond_ema.module if hasattr(vae_cond_ema, 'module') else vae_cond_ema
                    unwrapped_vae_pred_ema = vae_pred_ema.module if hasattr(vae_pred_ema, 'module') else vae_pred_ema
                    unwrapped_ema = ema.module if hasattr(ema, 'module') else ema
                    
                    c_feat_vis = unwrapped_vae_cond_ema.extract(c_init_vis).detach()
                    ...
                    z_sampled = fm_sampler.sample(batch_size=b_vis, cond=c_feat_vis, guidance_scale=args.cfg_scale)
                    running_mean_vis = unwrapped_ema.bn.running_mean.view(1, -1, 1).to(device)
                    running_var_vis = unwrapped_ema.bn.running_var.view(1, -1, 1).to(device)
                    z_sampled = z_sampled * torch.sqrt(running_var_vis + 1e-4) + running_mean_vis
                    
                    if hasattr(unwrapped_vae_pred_ema, 'decode_from_sample'):
                        pred_vis = unwrapped_vae_pred_ema.decode_from_sample(z_sampled)
                    else:
                        out_vis = unwrapped_vae_pred_ema.decode(z_sampled)
        ...
        if rank == 0 and (it + 1) % 10000 == 0:
            ...
            torch.save({
                ...
                'vae_pred_ema': vae_pred_ema.state_dict(),
                'vae_cond_ema': vae_cond_ema.state_dict(),
        ...
        if rank == 0 and (it + 1) % args.eval_freq == 0:
            ema.eval(); vae_cond_ema.eval(); vae_pred_ema.eval()
            
            run_evaluation(
                val_loader=val_loader, 
                ema=ema, 
                vae_cond_model=vae_cond_ema,
                vae_pred_model=vae_pred_ema, 
...
def parse_args(input_args=None):
    ...
    parser.add_argument("--wo_shift", action=argparse.BooleanOptionalAction, default=True)
    ...
    parser.add_argument('--cfg_scale', type=float, default=3.0, help='Classifier-free guidance scale used during FM sampling.')
