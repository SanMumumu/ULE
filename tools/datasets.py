...
class OpenDVDataset(ImageFramesDataset):
    def __init__(self, *args, **kwargs):
        # 1. 初始化父类
        super().__init__(*args, **kwargs)

        # 2. 打印调试信息，确认路径是否正确传入
        print(f"--- [OpenDV] Initializing dataset root: {self.path}")
    ...
    def load_video_paths(self):
        # == 步骤 A: 搜索图片 (尝试 jpg 和 png) ==
        # 使用绝对路径 self.path 进行搜索，glob 会返回绝对路径
        search_pattern_jpg = os.path.join(self.path, '*', '*.jpg')
        all_frames = sorted(glob(search_pattern_jpg))

        # 如果找不到 jpg，尝试 png
        if len(all_frames) == 0:
            print(f"--- [OpenDV] No .jpg files found. Trying .png...")
            search_pattern_png = os.path.join(self.path, '*', '*.png')
            all_frames = sorted(glob(search_pattern_png))

        # == 步骤 B: 检查是否为空 ==
        if len(all_frames) == 0:
            raise RuntimeError(
                f"ERROR: No images found in {self.path}!\n"
                f"Please check:\n"
                f"1. Did you set the path correctly in arguments?\n"
                f"2. Is the folder structure 'root/video_name/frame.jpg'?\n"
                f"3. Are the files .jpg or .png?"
            )
        
        print(f"--- [OpenDV] Found {len(all_frames)} frames. Example: {all_frames[0]}")

        # == 步骤 C: 处理视频片段 ==
        # 丢弃末尾不足 video_len (30帧) 的部分
        valid_frames_count = len(all_frames) - (len(all_frames) % self.video_len)
        all_frames = all_frames[:valid_frames_count]

        # 重组为视频列表 (这里存储的是【绝对路径】，读取时最安全)
        self.video_list = [all_frames[i:i + self.video_len] for i in
                           range(0, len(all_frames), self.video_len)]

        print(f"--- [OpenDV] Successfully loaded {len(self.video_list)} video clips.")