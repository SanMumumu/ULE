...
def main():
    ...
    for n, (x, _) in enumerate(tqdm(test_loader, desc="Evaluating", total=eval_batches)):
        ...
        for traj_idx in range(args.traj):
            ...
            z = z * torch.sqrt(running_var.to(torch.float32) + 1e-5) + running_mean.to(torch.float32)
            ...
            while pred.size(1) < args.future_frames:
                ...
                z_new = z_new * torch.sqrt(running_var.to(torch.float32) + 1e-5) + running_mean.to(torch.float32)
