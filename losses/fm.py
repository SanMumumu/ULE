...
class FlowMatching(nn.Module):
    ...
    def sample(self, batch_size=16, cond=None, guidance_scale=0.0):
        ...
        use_cfg = cond is not None and guidance_scale is not None and guidance_scale > 1.0
        if use_cfg:
        ...
        for i in range(self.sampling_timesteps):
            ...
            if use_cfg:
                ...
                t_double = torch.cat([scaled_t, scaled_t], dim=0)
                cond_double = torch.cat([cond, uncond], dim=0)

                v_double = self.model(x_double, cond_double, t_double)
                v_cond, v_uncond = v_double.chunk(2, dim=0)
                v_pred = v_uncond + guidance_scale * (v_cond - v_uncond)
            else:
                v_pred = self.model(x, cond, scaled_t)
